...
Authentication is restricted to a dedicated QA/test account. Its credentials are stored locally in a gitignored `.env.test` using `QA_LOGIN_EMAIL` and `QA_LOGIN_PASSWORD`, with owner-only permissions. It requires explicit human approval that names the target site, account purpose, and intended QA outcome before any credential entry or login action. The agent must not print, commit, or paste those values into chat, logs, or screenshots; it must not use a personal or production account, import cookies, read/export cookies or storage, use a personal browser profile, upload files, handle payments, or change account/security settings. Each later state-changing QA action remains subject to its own explicit approval.
